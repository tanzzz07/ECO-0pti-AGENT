...
parser = PydanticOutputParser(
    pydantic_object=TransportSuggestions
)
...
def calculate_transport_emissions(
    ...
    return round(
        total_km * EMISSION_FACTOR_KG_PER_KM,
        2
    )
...
def run_transport_agent(input_data):
    ...
    num_vehicles = input_data.get(
        "number_of_diesel_vehicles",
        0
    )

    avg_km_per_day = input_data.get(
        "average_km_per_vehicle_per_day",
        0
    )

    transport_emission = calculate_transport_emissions(
        num_vehicles,
        avg_km_per_day
    )
    ...
    try:
        ...
        print(
            f"Transport Agent LLM Response: {response}"
        )
